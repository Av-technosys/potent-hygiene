...
export const OrderDetailsPage = ({
  ...
  return (
    <>
      {orderReview == true ? (
        <OrderReview orderDetails={orderData} setOrderReview={setOrderReview} onBack={onBack}/>
      ):
      (
              ...
              orderData.status === "delivered" ? (
                <Button
                 onClick={() => router.push(`/invoice/${orderData.id}`)}
                  className="flex-1 cursor-pointer rounded-full h-12 bg-[#1D4E4E] text-white hover:bg-[#143a3a]"
                >
                  Invoice
                </Button>
              ) : (
