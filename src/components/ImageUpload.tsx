...
export default function ImageUpload({ onUploadSuccess, initialImage }: ImageUploadProps) {
  ...
  const uploadImage = async (selectedFile: File) => {
    ...
    try {
      ...
      // ImageKit/API se jo naya URL mila usey set karein
      setPreview(data.url);
        ...
        {preview ? (
          <div className="relative w-full h-full flex items-center justify-center">
             ...
             <div className="absolute inset-0 bg-black/5 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                <p className="text-xs bg-white/90 px-2 py-1 rounded shadow-sm font-medium">Change Image</p>
             </div>
          </div>
        ) : (
          <>
            <ImagePlus className="w-10 h-10 text-slate-400 mb-3" />
            <p className="text-sm font-medium text-slate-600">Click to upload category image</p>
            <p className="text-xs text-slate-400 mt-1">SVG, JPG, PNG (max. 2 MB)</p>
          </>
        )}
      </div>
