...
export default function CategoryProducts({ products, productsCategory }: any) {
  ...
  const productDetaials = useMemo(() => {
    return products.map((item: any) => {
      return { ...item, categories: productsCategory.filter((category: any) => category.productId === item.id).map((item: any) => item.categoryId) }
    });
  }, []);
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 h-full gap-6 flex-1">
      {
        filteredProducts.length > 0 ? filteredProducts?.map((value: any) => (
          <div
            key={value.id}
